"""
from math import * là một Anti-pattern vì:
    Khi đọc code sẽ khó biết hàm nào đến từ thư viện nào
    sẽ khiến dự án chạy chậm 
cách inpor an toàn và tường minh là :
    impor math 
        distance = math.sqrt(100)
hoặc : impor math sprt....
là chỉ impor nhưng thư viện cần dùng đến 

để biến một thư mục thành Package Cần tạo file:__init__.py
vai trò của __init__.py:
    Đánh dấu thư mục là một Package Python
    Cho phép import module từ thư mục đó
    Có thể dùng để khởi tạo package hoặc gom các module lại với nhau

cây thư mục:
Bài1/
|
|__ main.py
|
|__ core/
|   |__ __init__.py
|   |__ geo_calculator.py
|   |__ time_estimator.py
|
|__ utils/
|   |__ __init__.py
|   |__ file_helper.py
|
|__ logs/
"""

from datetime import datetime

from core.geo_calculator import calculate_distance
from core.time_estimator import predict_eta
from utils.file_helper import create_log_dir


shipments = [
    {
        "id": "TRK-001",
        "from_lat": 21.0285,
        "from_lon": 105.8542,
        "to_lat": 10.8231,
        "to_lon": 106.6297,
        "depart": "2026-06-10 08:00:00",
        "deadline": "2026-06-11 12:00:00"
    },
    {
        "id": "TRK-002",
        "from_lat": 21.0285,
        "from_lon": 105.8542,
        "to_lat": 16.0544,
        "to_lon": 108.2022,
        "depart": "2026-06-10 09:30:00",
        "deadline": "2026-06-10 15:00:00"
    }
]


print("====== HỆ THỐNG ĐIỀU PHỐI RIKKEI LOGISTICS =======")

create_log_dir("logs")

print("[INFO] Khởi tạo hệ thống lưu trữ log hành trình... Thành công.")
print("-" * 75)

for shipment in shipments:

    distance = calculate_distance(
        shipment["from_lat"],
        shipment["from_lon"],
        shipment["to_lat"],
        shipment["to_lon"]
    )

    eta = predict_eta(
        shipment["depart"],
        distance
    )

    deadline = datetime.strptime(
        shipment["deadline"],
        "%Y-%m-%d %H:%M:%S"
    )

    print(f"[CHUYẾN XE {shipment['id']}]")
    print(f" + Khoảng cách vận chuyển: {distance:.2f} km")
    print(f" + Thời gian khởi hành: {shipment['depart']}")
    print(f" + Dự kiến cập bến (ETA): {eta}")

    if eta <= deadline:
        print(
            " + Trạng thái: 🟢 AN TOÀN (Kịp tiến độ trước deadline)"
        )
    else:
        print(
            f" + Trạng thái: 🔴 CẢNH BÁO (Trễ hạn! Deadline yêu cầu lúc "
            f"{deadline.strftime('%H:%M:%S')})"
        )

    print()

print("=" * 56)